export const fr = {
  'My Account': 'Mon compte',
  Welcome: 'Bienvenue',
  'My Moyen Account': 'Mon compte moyen',
  'My Profile': 'Mon profil',
  Orders: 'Ordres',
  'Saved Items': 'Articles enregistrés',
  'My Settings': 'Mes paramètres',
  'Change Password': 'Changer le mot de passe',
  Logout: 'Se déconnecter',
  'Contact With Admin': "Contacter l'administrateur",
  'Warning!': 'Avertissement',
  'Are you sure, yout want to logout from this device':
    'Êtes-vous sûr de vouloir vous déconnecter de cet appareil.',
  'User Profile': "Profil de l'utilisateur",
  'Account Details': 'Détails du compte',
  'Username *': "Nom d'utilisateur *",
  'Phone Number *': 'Numéro de téléphone *',
  'City *': 'Ville *',
  'Address One *': 'Adresse Un *',
  'Address Two *': 'Adresse Deux *',
  'Email address *': 'Adresse e-mail *',
  'Zip Code *': 'Code postal *',
  'Country *': 'Pays *',
  'Update Profile': 'Mettre à jour le profil',
  'Kindly login first': "Veuillez d'abord vous connecter",
  'This field can not be empty': 'ce champ ne peut pas être vide',
  'Profile Updated Successfully': 'Mise à jour du profil réussie',
  'Change Your Password': 'Changez votre mot de passe',
  Notice: 'Avis',
  "It's a good idea to use a strong password that you're not using elsewhere":
    "C'est une bonne idée d'utiliser un mot de passe fort que vous n'utilisez pas ailleurs.",
  'Current Password *': 'Mot de passe actuel *',
  'New Password *': 'Nouveau mot de passe *',
  'Confirm New Password *': 'Confirmer le nouveau mot de passe *',
  Yes: 'Oui',
  No: 'Non',
  'Help For user To Contact Super Admin':
    "Aide Pour l'utilisateur Pour contacter le super administrateur",
  'Top sellers': 'meilleures ventes',
  'See All': 'Voir tout',
  'New Arrivals': 'Nouvelles Arrivées',
  Brands: 'Marques',
  'Categories :': 'Catégories :',
  'Price :': 'Prix :',
  'Apply Filter': 'Appliquer le filtre',
  'Clear Filter': 'Effacer le filtre',
  'Please type something to search':
    'Veuillez taper quelque chose pour rechercher...',
  'Issue while fetching categories':
    'Problème lors de la récupération des catégories.',
  Category: 'Catégorie',
  'See All Product': 'Voir tous les produits',
  'You have no items in this list':
    "Vous n'avez aucun élément dans cette liste",
  Featured: 'Mis en exergue',
  'Select Payment Method': 'Sélectionnez le mode de paiement',
  Wishlist: 'Liste de souhaits',
  'Search Items': 'Rechercher des éléments',
  'All Products': 'Tous les produits',
  Carts: 'Chariots',
  'Select All': 'Tout sélectionner',
  Remove: 'Éliminer',
  Subtotal: 'Total',
  Total: 'Totale',
  'Billing Address': 'adresse de facturation',
  'Cart Deleted': 'Panier supprimé',
  'Your Cart Has been deleted': 'Votre panier a été supprimé',
  'Complete your order': 'Complétez votre commande',
  'Kindly select at least 1 item': 'Veuillez sélectionner au moins 1 élément',
  'Are you sure you want to delete all product ?':
    'Êtes-vous sûr de vouloir supprimer tous les produits ?',
  'You have no items in the cart': "Vous n'avez aucun article dans le panier",
  'Add items you want to shop':
    'Ajoutez les articles que vous souhaitez acheter',
  'Continue Shopping': 'Continuer vos achats',
  Filter: 'Filtre',
  'Please Enter correct Search': 'Veuillez entrer la recherche correcte',
  Search: 'Recherche...',
  'Added to wishlist': 'Ajouté à la liste de souhaits',
  'This item has been removed from your wishlist':
    'Cet article a été supprimé de votre liste de souhaits',
  'No product found': 'Aucun produit trouvé',
  'Kindly search to see products listings':
    'Veuillez rechercher pour voir les listes de produits',
  'Something went wrong.': "Quelque chose s'est mal passé.",
  'Remove from Cart!': 'Retirer du panier !',
  'Are you sure you want to remove item from your cart':
    "Êtes-vous sûr de vouloir supprimer l'article de votre panier.",
  'Description :': 'La description :',
  'Vendor:': 'Vendeuse:',
  'Store Name:': 'Nom du magasin:',
  'Address:': 'Adresse:',
  'Phone:': 'Téléphoner:',
  'Submit Your Review': 'Soumettez votre avis',
  'Your email address will not be published Required fields are marked':
    'Votre adresse email ne sera pas publiée. Les champs requis sont indiqués *',
  Submit: 'Soumettre',
  'No reviews yet': "Pas encore d'avis.",
  'Out of stock': 'Rupture de stock',
  'Write Your Reviews Here': 'Écrivez vos commentaires ici..',
  'Enter Your Name*': 'Entrez votre nom*',
  'Enter Your Email*': 'Entrer votre Email*',
  Follow: 'Suivre',
  Unfollow: 'Ne plus suivre',
  Details: 'Des détails',
  'Related Products': 'Produits connexes',
  'Kindly login to give a review':
    'Veuillez vous connecter pour donner un avis.',
  'Kindly select the product attributes before adding to cart':
    "Veuillez sélectionner les attributs du produit avant de l'ajouter au panier.",
  'Some thing is wrong': 'Quelque chose ne va pas.',
  'Your Product Has Been Add To Cart': 'Votre produit a été ajouté au panier',
  'Add To Cart': 'Ajouter au panier',
  'Email *': 'E-mail *',
  'Password *': 'Mot de passe *',
  Login: 'Connexion',
  'New on Moyen?': 'Nouveau sur Moyen ?',
  'Forgot password?': 'mot de passe oublié?',
  'Create Account': 'Créer un compte',
  PrivacyPolicy: 'Politique de confidentialité',
  Number: 'Numéro *',
  'Confirm Password *': 'Confirmez le mot de passe *',
  'Already Have An Account ?': 'Vous avez déjà un compte ?',
  Signup: 'Inscrivez-vous',
  Home: 'Domicile',
  Shop: 'Boutique',
  Cart: 'Chariot',
  Account: 'Compte',
  Delivery: 'Livraison',
  Summary: 'Résumé',
  Payment: 'Paieent',
  'Full Name': 'Nom complet *',
  'ZipCode *': 'Code postal *',
  Note: 'Noter',
  'Order Items': 'Items commandés',
  Proceed: 'Procéder',
  'Back To Home': 'De retour à la maison',
  Back: 'Arrière',
  'Would like to same address from your last one':
    'Je voudrais la même adresse que la dernière',
  Success: 'Succès',
  'Please first complete all shipping details':
    'Please first complete all shipping details',
  'Same as Shipping Address': "Même que l'adresse d'expédition",
  'Please enter all delivery information':
    'Veuillez entrer toutes les informations de livraison',
  'Contact With Owner': 'Contact avec le propriétaire',
  'For complete your order please contact the owner!':
    "Pour compléter votre commande s'il vous plaît contacter le propriétaire!",
  'Order Details': 'détails de la commande',
  'Product Details': 'détails du produit',
  'Shipping Details': "Les détails d'expédition",
  'Return Order': 'Commande de retour',
  'Cancel Order': 'annuler la commande',
  'Are you sure want to cancel this order':
    'Voulez-vous vraiment annuler cette commande ?',
  'Return policy are not available':
    "La politique de retour n'est pas disponible. \n Pour plus d'informations, veuillez contacter le super administrateur.",
  Close: 'proche',
  'You have no orders to display': "Vous n'avez aucune commande à afficher.",
  'You canceled your order': 'Vous avez annulé votre commande',
  'You already cancelled your order': 'Vous avez déjà annulé votre commande',
  'My Orders': 'Mes commandes',
  'Check Out': 'Vérifier',
  'The email has already been taken.': 'Le couriel a déja été pris en compte.',
  'The phone number has already been taken.':
    'Le numéro de téléphone a déjà été pris.',
  Register: "S'inscrire",
  Attributes: 'Les attributs',
  'Attributes :': 'Les attributs',
  'Select Attribute': "Sélectionnez l'attribut",
  'Language/Currency': 'Langue/Devise',
  LanguageChange: 'Changement de langue',
  'Currency Change': 'Changement de devise',
  'Welcome To MoyenXpress': 'Bienvenue sur MoyenXpress.',
  'Best Online Shopping Platform': "Meilleure plateforme d'achat en ligne.",
  'Best Shopping Experience': 'Meilleure expérience de magasinage.',
  'Let you shop milions of products with best online shopping Deals':
    "Laissez-vous acheter des millions de produits avec les meilleures offres d'achat en ligne.",
  'Best Reasonable Price': 'Meilleur prix raisonnable.',
  'An exciting place for the whole family to shop':
    'Un endroit passionnant pour toute la famille pour magasiner',
  test: 'test',
};
